...
def pareto(costs: np.ndarray):
    ...
    for i, c in enumerate(costs):
        is_pareto[i] = np.all(np.any(costs[:i]>c, axis=1)) and np.all(np.any(costs[i+1:]>c, axis=1))
    return is_pareto
...
def nDS(costs):
    ...
    fronts = []
    while al.shape[0]>0:
        args = np.argwhere(pareto(al)).flatten().tolist()
        fronts.append(al[args])
        al = np.delete(al, args,axis=0)
    return fronts
